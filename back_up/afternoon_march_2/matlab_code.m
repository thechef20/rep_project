...
upper_counter =0;
lower_counter = 0;
pre_rank_matrix = nan(length(stock_symbols_refrences),7); %if I wanted individual beta we would extend this to 6
for i =1:length(stock_symbols_refrences)-1
    current_stock = stock_symbols_refrences(i);
    next_stock =stock_symbols_refrences(i+1);
    r_and_m_name = [stock_year_data(i)  stock_month_data(i) current_stock individual_stock_market_cap(i) individual_stock_return(i) risk_free(i)];%year and month and name
...
%%
pre_rank_table = array2table(pre_rank_matrix,'VariableNames',{'annual','mmonth','kypermno','mtcap','mretx','risk_free','pre_ranked_beta'});%we changed myear to annual because of the porflio data uses annual
